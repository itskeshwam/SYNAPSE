...
def search_images(query: str, per_page: int = 1) -> dict:
    ...
    Args:
        query: The topic to search for (e.g., 'Tokyo skyline', 'semiconductors').
    ...
    try:
        ...
        # Format the results to be cleaner for the agent
        results = []
        for photo in photos:
            results.append({
                "id": photo.get("id"),
                ...
                "alt": photo.get("alt"),
                # "src": photo.get("src", {}).get("large") # We'll return the 'large' version
                "src": {
